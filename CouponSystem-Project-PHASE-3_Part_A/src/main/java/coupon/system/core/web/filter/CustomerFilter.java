...
public class CustomerFilter implements Filter {
	...
	public CustomerFilter(SessionContext sessionContext) {
		super();
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (token != null) {
			...
			if (session != null) {
				...
				Object customerService = session.getAttributes("service");
				...
				if (customerService instanceof CustomerService) {
					...
					resp.sendError(HttpStatus.UNAUTHORIZED.value(), "You are not Customer - Blocking the request");
					return;
				}
			}
		}
